...
function Wo_Login($username, $password) {
    ...
    //echo "SELECT * FROM " . T_USERS . " WHERE (`username` = '{$username}' OR `email` = '{$username}' OR `phone_number` = '{$username}')";
	...
	//print_r($mysqli_hash_upgrade); exit;
	$_SESSION['custom_utype']= $mysqli_hash_upgrade['custom_utype'];
    $hash                = 'md5';
...
function Wo_GetSearchFilter($result, $limit = 30, $offset = 0) {
    ...
    if (!empty($result['custom_utype'])) {
        $result['custom_utype'] = Wo_Secure($result['custom_utype']);
    }
    ...
    if (!empty($result['custom_utype'])) {
		$query .= " AND (`custom_utype` = '".$result['custom_utype']."') ";
    }
...
function Wo_ShareFile($data = array(), $type = 0, $crop = true) {
    ...
    if ($wo['config']['fileSharing'] == 1) {
        ...
        $allowed = 'jpg,png,jpeg,gif,mp4,m4v,webm,flv,mov,mpeg,mp3,wav,doc,docx';
    }
    $new_string        = pathinfo($data['name'], PATHINFO_FILENAME) . '.' . strtolower(pathinfo($data['name'], PATHINFO_EXTENSION));
    $extension_allowed = explode(',', $allowed);
    $file_extension    = pathinfo($new_string, PATHINFO_EXTENSION);
...
function Wo_GetPosts($data = array('filter_by' => 'all', 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 5, 'event_id' => 0, 'ad-id' => 0)) {
		...
		if(empty($_SESSION['everyone'])){
			...
			//if ($add_filter_query == true) {
				...
				AND (
					  ...
					  AND `user_id` IN (SELECT `following_id` FROM " . T_FOLLOWERS . " WHERE `follower_id` = {$logged_user_id} AND `active` = '1')
					  )
				)";
			//}
		}
	...
	if(!empty($_SESSION['rpcat'])){
		$query_text .= " AND `postCategory` = '".$_SESSION['rpcat']."' ";
	}
...
function Wo_ProfileCompletion(){
	...
	$user_id = Wo_Secure($user_id);
    $logged_user_id = Wo_Secure($wo['user']['user_id']);
    $query = " SELECT * FROM " . T_USERS_FIELDS . " WHERE  user_id='".$logged_user_id."'";
    $sql_query = mysqli_query($sqlConnect, $query);
	$fetched_setting_data = mysqli_fetch_assoc($sql_query);
    if (!empty($fetched_setting_data['fid_2'])) {
        $data[3] = 25;
    }
...
//    }
	...
	if (!empty($fetched_setting_data['fid_3'])) {
        $data[5] = 25;
    }
