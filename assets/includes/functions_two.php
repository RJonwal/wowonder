...
function Wo_GetSavedPosts($user_id, $after_post_id = 0, $limit = 0)
{
    ...
    if (empty($_SESSION['rpcat'])) {
        ...
        $reqcat = " and wop.postCategory='" . $_SESSION['rpcat'] . "' ";
    }
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $data           = array();
    //$query_one = "SELECT `post_id` FROM " . T_SAVED_POSTS . " WHERE `user_id` = {$user_id} "; //T_POSTS
    $query_one = "SELECT * FROM " . T_SAVED_POSTS . " wos INNER JOIN " . T_POSTS . " wop ON wos.`post_id` = wop.`post_id` WHERE wos.user_id={$user_id} " . $reqcat;
    if (isset($after_post_id) && !empty($after_post_id) && is_numeric($after_post_id)) {
        $after_post_id = Wo_Secure($after_post_id);
        $query_one .= " AND wos.post_id < {$after_post_id}";
    }
    $query_one .= " ORDER BY wos.`id` DESC";
...
function Wo_GetTrendingPosts($user_id, $after_post_id = 0, $limit = 0)
{
    global $wo, $sqlConnect;
    if (empty($_SESSION['rpcat'])) {
        $reqcat = '';
    } else {
        $reqcat = " and post.postCategory='" . $_SESSION['rpcat'] . "' ";
    }
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    $query_one = "SELECT
    post.*, 
    COUNT(likes.id) AS tlikes
FROM
    `" . T_POSTS . "` post
LEFT JOIN
    " . T_LIKES . " likes
    ON (likes.post_id = post.post_id) " . $reqcat . " 
GROUP BY
    likes.post_id ";
    $query_one .= " ORDER BY tlikes DESC";
    if (isset($limit) && !empty($limit) && is_numeric($limit)) {
        $query_one .= " LIMIT {$limit}";
    }
    //echo $query_one;
...
function Wo_AddressData($group_id = 0)
{
    global $wo, $sqlConnect, $cache;
    ...
    $query_one       = "SELECT * FROM wo_address_book WHERE `id` = {$group_id}";
    $hashed_group_Id = md5($group_id);
    if ($wo['config']['cacheSystem'] == 1) {
        $fetched_data = $cache->read($hashed_group_Id . '_Address_Data.tmp');
        if (empty($fetched_data)) {
            $sql          = mysqli_query($sqlConnect, $query_one);
            $fetched_data = mysqli_fetch_assoc($sql);
            $cache->write($hashed_group_Id . '_Address_Data.tmp', $fetched_data);
        }
    } else {
        $sql          = mysqli_query($sqlConnect, $query_one);
        $fetched_data = mysqli_fetch_assoc($sql);
    ...
    $fetched_data['company'] = $fetched_data['company'];
    $fetched_data['phone_number']    = $fetched_data['phone_number'];
    $fetched_data['email']        = $fetched_data['email'];
    $fetched_data['about'] = $fetched_data['about'];
    return $fetched_data;
}
function Wo_GetMyAddress()
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $data       = array();
    $user_id    = Wo_Secure($wo['user']['user_id']);
    $query_text = "SELECT `id` FROM wo_address_book WHERE `user` = {$user_id} order by name asc";
    $query_one  = mysqli_query($sqlConnect, $query_text);

    while ($fetched_data = mysqli_fetch_assoc($query_one)) {
        if (is_array($fetched_data)) {
            $data[] = Wo_AddressData($fetched_data['id']);
        }
    }

    return  $data;
}
function Wo_GetMyNotes()
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $data       = array();
    $user_id    = Wo_Secure($wo['user']['user_id']);
    // $query_text = "SELECT `id` FROM wo_note_pad WHERE `cby` = {$user_id} order by cdate desc";
    $query_text =  "SELECT `id` FROM (SELECT * FROM wo_note_pad  WHERE `cby` = {$user_id} ORDER BY cdate desc ) AS sub GROUP BY title";
    
    $query_one  = mysqli_query($sqlConnect, $query_text);

    while ($fetched_data = mysqli_fetch_assoc($query_one)) {
        if (is_array($fetched_data)) {
            $data[] = Wo_NoteData($fetched_data['id']);
        }
    }
...
function Wo_NoteData($noteId = 0)
{
    global $wo, $sqlConnect, $cache;


    $query_one       = "SELECT * FROM wo_note_pad WHERE `id` = {$noteId}";
    $sql          = mysqli_query($sqlConnect, $query_one);
    $fetched_data = mysqli_fetch_assoc($sql);
    ...
    $fetched_data['id']    = $fetched_data['id'];
    $fetched_data['title']        = $fetched_data['title'];
    $fetched_data['note'] = $fetched_data['note'];
    $fetched_data['cby']    = $fetched_data['cby'];
    $fetched_data['cip']        = $fetched_data['cip'];
    $fetched_data['cstatus'] = $fetched_data['cstatus'];
    $fetched_data['cdate'] = $fetched_data['cdate'];
...
function Wo_GetGroupPostPublisherBox($group_id = 0)
{
    ...
    if (Wo_CanBeOnGroup($group_id) === true) {
        ...
        if ($wo['user']['user_id'] == $group['user_id']) {
            $continue = true;
        }
    }
    if ($continue == true) {
...
{
    global $sqlConnect;
    $search_qeury_data = ($search_qeury);
    if (empty($search_qeury['query'])) {
        $search_qeury = Wo_Secure($search_qeury);
    } else {
        $search_qeury = Wo_Secure($search_qeury_data['query']);
    }
    $data         = array();
    $offset_to    = "";
    if ($type == 'groups') {
        ...
        $querytxt = " SELECT `id` FROM " . T_POSTS . " WHERE `postText` LIKE '%$search_qeury%' ";

        if ($search_qeury_data['postCategory'] > 0) {
            $querytxt .= " and postCategory='" . $search_qeury_data['postCategory'] . "' ";
        }
        if ($search_qeury_data['postType']  == "postPhotos") {
            $querytxt .= " and postFile like 'upload/photos/%' ";
        }
        if ($search_qeury_data['postType']  == "postVideo") {
            $querytxt .= " and postFile like 'upload/videos/%' ";
        }
        if ($search_qeury_data['postType']  == "postFile") {
            $querytxt .= " and postFile like 'upload/files/%' ";
        }
        if ($search_qeury_data['postType']  == "postPoll") {
            $querytxt .= " and id in (SELECT post_id FROM " . T_POLLS . " GROUP by post_id) ";
        }
        $querytxt .= "  ORDER BY `id` DESC LIMIT 10 ";
        $query = mysqli_query($sqlConnect, $querytxt);
        while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function Wo_SearchFor($search_qeury, $type, $offset = 0)
{
    ...
    if ($type == 'group') {
        ...
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $new_data['label'] = $group['group_title'];
            $new_data['img'] = $group['avatar'];
            $data[] = $new_data;
        }
        //Getting joined groups
        $query     = " SELECT `group_id` FROM " . T_GROUP_MEMBERS . " WHERE `user_id` = '" . $wo['user']['id'] . "' AND `active` = '1'";
        $sql_query = mysqli_query($sqlConnect, $query);
        while ($fetched_data = mysqli_fetch_assoc($sql_query)) {
            $group = Wo_GroupData($fetched_data['group_id']);
            $new_data = array();
            $new_data['id'] = $group['id'];
            $new_data['label'] = $group['group_title'];
            $new_data['img'] = $group['avatar'];
...
function check_differenceDate($d1, $d2, $type="gt"){
    $dn1 = new DateTime($d1);
	$dn2 = new DateTime($d2);
    ...
    // greater than dates
    if($type == 'gt'){ if($dn1 > $dn2){  $flg = true; } }

    // Less than dates
    else if($type == 'lt'){ if($dn1 < $dn2){  $flg = true; } }

    // equal dates
    else if($type == 'eq'){ if($dn1 == $dn2){  $flg = true; } }

    // less than or equal
    else if($type == 'lte'){ if($dn1 <= $dn2){  $flg = true; } }

    // greater than or equal
    else if($type == 'gte'){ if($dn1 >= $dn2){  $flg = true; } }
    return $flg;
}

function removeOneTimeProMemberShip()
{
    global $sqlConnect;
    $data      = array();
    $query_one = "SELECT `user_id`, `pro_type`, `pro_time`, `payment_type`, `pro_expire_date` FROM " . T_USERS . " WHERE `is_pro` = '1' AND `payment_type` = 'one-time' AND `pro_expire_date` IS NOT NULL ORDER BY `user_id` ASC";
    $sql       = mysqli_query($sqlConnect, $query_one);
    while ($fetched_data = mysqli_fetch_assoc($sql)) {
        $update_data = false;
        $paymentType = $fetched_data['payment_type'];

        if($paymentType == 'one-time'){
            $expire_date    = $fetched_data['pro_expire_date'];
            $current_date   = date('d-m-Y');

            $checkProExpire = check_differenceDate($current_date, $expire_date, 'gt');
            ...
            if ($update_data == true) {
                ...
                $update      = Wo_UpdateUserData($fetched_data['user_id'], $updateUser);
                $user_id     = $fetched_data['user_id'];
                $mysql_query = mysqli_query($sqlConnect, "UPDATE " . T_PAGES . " SET `boosted` = '0' WHERE `user_id` = {$user_id}");
                $mysql_query = mysqli_query($sqlConnect, "UPDATE " . T_POSTS . " SET `boosted` = '0' WHERE `user_id` = {$user_id}");
                $mysql_query = mysqli_query($sqlConnect, "UPDATE " . T_POSTS . " SET `boosted` = '0' WHERE `page_id` IN (SELECT `page_id` FROM " . T_PAGES . " WHERE `user_id` = {$user_id})");
