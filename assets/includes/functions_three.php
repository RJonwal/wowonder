...
function Wo_GetDiscountCodes() {

    global $sqlConnect;

    $data          = array();

    $query_one     = "SELECT * FROM " . T_DISCOUNT_CODES . " ORDER BY `id` DESC";
...
function Wo_GetDiscountCode($id) {
    ...
    $query_one     = "SELECT * FROM " . T_DISCOUNT_CODES . " WHERE `id` = '{$id}'";
...
function Wo_RegisterNewDiscountCode($registration_data) {
    ...
    $query  = mysqli_query($sqlConnect, "INSERT INTO " . T_DISCOUNT_CODES . " ({$fields}) VALUES ({$data})");
...
function Wo_DeleteDiscountCode($id) { 
    ...
    $query = mysqli_query($sqlConnect, "DELETE FROM " . T_DISCOUNT_CODES . " WHERE `id` = {$id}");
...
function Wo_UpdateDiscountCodeData($id, $update_data) {
    ...
    $query_one = "UPDATE " . T_DISCOUNT_CODES . " SET {$impload} WHERE `id` = {$id} ";
...
function Wo_DeleteMyBlog($id = 0) {
    ...
    if ($sql_query_one) {
		...
		//$rowcount=mysqli_num_rows($sql_query_two);
		//if($rowcount>0){
...
function Wo_AddressBook($registration_data = array()) {
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }else{
    ...
    $fields = '`' . implode('`, `', array_keys($registration_data)) . '`';
    $data   = '\'' . implode('\', \'', $registration_data) . '\'';
    $query  = mysqli_query($sqlConnect, "INSERT INTO wo_address_book ({$fields}) VALUES ({$data})");
    if ($query) {
        return true;
    }
    return false;
...
function Wo_saveMyNote($nodePadData = array()) {
    ...
    $fields = '`' . implode('`, `', array_keys($nodePadData)) . '`';
    $data   = '\'' . implode('\', \'', $nodePadData) . '\'';
    $query  = mysqli_query($sqlConnect, "INSERT INTO wo_note_pad ({$fields}) VALUES ({$data})");
    if ($query) {
        return true;
    }
    return false;
...

{
    ...
    foreach ($update_data as $field => $data) {
        ...
        if ($field == 'price' || $field == 'featured_member' ||  $field == 'profile_visitors' ||  $field == 'last_seen' /* ||  $field == 'verified_badge' */ || $field == 'posts_promotion' ||  $field == 'pages_promotion' ||  $field == 'discount' ||  $field == 'image' || $field == 'autoconsult' || $field == 'status' || $field == 'time' || $field == 'stripe_price_id' ||  $field == 'stripe_product_id' ) {
...
function Wo_isAddressIdExists($id) {
    ...
    $query_one = mysqli_query($sqlConnect, "SELECT * FROM wo_address_book WHERE `id` =".$id);
   

    $sql_query_one = mysqli_fetch_assoc($query_one);

    if (isset($sql_query_one) && is_array($sql_query_one) && count($sql_query_one) > 0) {
...
function Wo_isNoteIdExists($id) {
    ...
    $query_one = mysqli_query($sqlConnect, "SELECT * FROM wo_note_pad WHERE `id` =".$id);
...
function Wo_copyAutoConsultEntries($newUserId)
{
    require "function/function.php";

    global $sqlConnect, $wo;

    if (empty($newUserId) || !is_numeric($newUserId) || $newUserId < 1) {

        return false;
    }
    $newUserId = Wo_Secure($newUserId);
    $genericUserId = Wo_UserIdFromUsername(GENERIC_USERNAME);

    if (empty($genericUserId) || !is_numeric($genericUserId) || $genericUserId < 1) {
        return false;
    }
    ...
    $query_1 = mysqli_query($sqlConnect, "SELECT * from category_details where cby=$genericUserId AND parent_id=0"); 
    while ($category = mysqli_fetch_assoc($query_1)) {
        $insertVal = mysqli_query($sqlConnect, "INSERT INTO category_details (`category_name`, `parent_id`, `mark_favourite`, `cby`, `cstatus`, `cip`, `cdate`, `dorder`, `bgcolor`, `replica_id`) VALUES ('$category[category_name]', 0, $category[mark_favourite], $newUserId, $cstatus , '$cip', '$cdate', $category[dorder], '$category[bgcolor]', $category[id])");        
        $cat_id = mysqli_insert_id($sqlConnect);

        $query_2 = mysqli_query($sqlConnect, "SELECT * from category_details where cby=$genericUserId AND parent_id=".$category['id']);
        while ($subCategory = mysqli_fetch_assoc($query_2)) {
            $insertVal = mysqli_query($sqlConnect, "INSERT INTO category_details (`category_name`, `parent_id`, `mark_favourite`, `cby`, `cstatus`, `cip`, `cdate`, `dorder`, `bgcolor`, `replica_id`) VALUES ('$subCategory[category_name]', $cat_id, $subCategory[mark_favourite], $newUserId, $cstatus , '$cip', '$cdate', $subCategory[dorder], '$subCategory[bgcolor]', $subCategory[id])");
            $sub_cat_id = mysqli_insert_id($sqlConnect);

            $query_3 = mysqli_query($sqlConnect, "SELECT * from category_details where cby=$genericUserId AND parent_id=".$subCategory['id']);
            while ($subsubCategory = mysqli_fetch_assoc($query_3)) {
                $insertVal = mysqli_query($sqlConnect, "INSERT INTO category_details (`category_name`, `parent_id`, `mark_favourite`, `cby`, `cstatus`, `cip`, `cdate`, `dorder`, `bgcolor`, `replica_id`) VALUES ('$subsubCategory[category_name]', $sub_cat_id, $subsubCategory[mark_favourite], $newUserId, $cstatus , '$cip', '$cdate', $subsubCategory[dorder], '$subsubCategory[bgcolor]', $subsubCategory[id])");        
                $sub_sub_cat_id = mysqli_insert_id($sqlConnect);

                if(isset($sub_sub_cat_id) && $sub_sub_cat_id > 0 ){            
                    $query_two = mysqli_query($sqlConnect, "SELECT * from preset_details where cat_id=".$subsubCategory['id']." AND cby =" . $genericUserId);
                    $preSets = array();
        ...
        if(isset($cat_id) && $cat_id > 0 ){            
            $query_two = mysqli_query($sqlConnect, "SELECT * from preset_details where cat_id=".$category['id']." AND cby =" . $genericUserId);
            $preSets = array();
            while ($preset = mysqli_fetch_assoc($query_two)) {                
                $insertVal = mysqli_query($sqlConnect, "INSERT INTO preset_details (`cat_id`, `presettxt`, `mark_favourite`, `cby`, `cip`, `cstatus`, `cdate`, `dorder`) VALUES ($cat_id, '$preset[presettxt]', $preset[mark_favourite], $newUserId, '$cip', $cstatus ,'$cdate', $preset[dorder])");                
            }
        }
    }
    // $query_one = mysqli_query($sqlConnect, "INSERT into category_details(category_name, parent_id, mark_favourite, cby, cstatus, cip, cdate, dorder, bgcolor) select category_name, parent_id, mark_favourite, $newUserId, $cstatus, '$cip', '$cdate', dorder, bgcolor from category_details where cby =" . $genericUserId);
    
    // $query_two = mysqli_query($sqlConnect, "INSERT into preset_details(cat_id, presettxt, mark_favourite, cby, cip, cstatus, cdate, dorder) select cat_id, presettxt, mark_favourite, $newUserId, '$cip', $cstatus , '$cdate', dorder from preset_details where cby =" . $genericUserId);

        return true;
}

function checkAutoConsultAccess(){
    global $wo;
    ...
    if(($wo['user']['is_pro'] == '1' && !$checkProExpire) || (Wo_IsAdmin() || Wo_IsModerator())){
        return true;
    }
    return false;
}

function printr($val,$isDie=1){
