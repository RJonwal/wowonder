...
function Wo_LoadPage($page_url = '')
{
    ...
    require($page);
    // require('./themes/wowonder/layout/note-pad/my-notepad.phtml');
...
function Wo_SeoLink($query = '')
{
    ...
    if ($wo['config']['seoLink'] == 1) {
        $query = preg_replace(array(
            '/^index\.php\?link1=edit-note-pad&npid=([A-Za-z0-9_]+)$/i',
            '/^index\.php\?link1=edit-address-book&abid=([A-Za-z0-9_]+)$/i',
            ...
            $config['site_url'] . '/edit-note-pad/$1',
            $config['site_url'] . '/edit-address-book/$1',
...
function WoUserTypename($type)
{
    $type_id1 = WoUserTYpe();
    if ($type == 0) {
        return 'NA';
    }
    return $type_id1[$type];
}
function WoRegistrationNo()
{
    return array(
        "0" => "N/A",
        "1" => "GMC",
...
function WoRegistrationNoName($type)
{
    $type_id1 = WoUserTYpe();
    return $type_id1[$type];
}
