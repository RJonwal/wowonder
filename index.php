...
//     }
// }
$wo['genders'] = array("male" => "Male", "female" => "Female", "not-selected" => "Not Selected");
if (!empty($_REQUEST['cat'])) {
    $_SESSION['rpcat'] = $_REQUEST['cat'];
}
if (isset($_REQUEST['cat']) && $_REQUEST['cat'] == 0) {
    unset($_SESSION['rpcat']);
}
if ($wo['loggedin'] == true) {
...
if (!empty($_GET['ref']) && $wo['loggedin'] == false && !isset($_COOKIE['src'])) {
    ...
    if (!isset($_SESSION['ref']) && !empty($get_ip)) {
        ...
        if (!empty($user_date)) {
            ...
            //coding for session
            /*if(!empty($_SESSION['custom_utype'])){
			record_set("get_id","select user_id,custom_utype, from  wo_users where user_id ='".$_SESSION['custom_utype']."'");
			if($totalRows_get_id){
				$row_get_id= mysqli_fetch_assoc($get_id);
		echo 'registered Successfully';	
		  $_SESSION['user_id']= $row_get_id['user_id'];
        
          $_SESSION['custom_utype']=$row_get_id['custom_utype'];
			$mess = "Login Successfully";	
		reDirect('index.php?msg='.$mess); 
	}else{
	
		$mess ='username and password is wrong';
		reDirect('register.php?msg='.$mess);
	}
			}*/
            //
        }
...
$allowed_pages = array(
    "forgot-password", "register", "user-activation", "activate", "unusual-login", "reset-password", "success", "activated", "privacy-policy", "contact-us", "terms"
);
...
switch ($page) {
    ...
    case 'create-group':
        ...
        break;
    case 'addressbook':
        include('sources/my_addressbook.php');
        break;
    case 'address-book':
        include('sources/address_book.php');
        break;
    case 'edit-address-book':
        include('sources/edit_address_book.php');
        break;
        
    case 'notepad':
        include('sources/my_notepad.php');
        break;
    case 'note-pad':
        include('sources/note_pad.php');
        break;
    case 'edit-note-pad':
        include('sources/edit_note_pad.php');
        break;
    ...
    case 'saved-posts':
        ...
        break;
    case 'trending-posts':
        include('sources/trendingPosts.php');
        break;
    case 'hashtag':
    ...
    case 'common_things':
        ...
        break;
    case 'auto_consult':
        include('sources/auto_consult.php');
        break;
    case 'funding':
...
//print_r($_SESSION);
//echo $a =  'select * from wo_appssessions where session_id ="'.$_SESSION['user_id'].'"';
//$b = 	mysqli_query($sqlConnect,$a);		
//$c = mysqli_fetch_assoc($b);
//print_r($c);

//echo $custom = 'select * from wo_users where user_id ="'.$c['user_id'].'"';	
////$custom_a = mysqli_query($sqlConnect,$custom);	
//$custom_b = mysqli_fetch_assoc($custom_a);		
//$_SESSION['custom_utype']= $custom_b['custom_utype'];



/*$search_a ='select * from wo_users where user_id="'.$_SESSION['user_id'].'"';
	$s_a = mysqli_query($sqlConnect,$search_a);
	$s_b = mysqli_fetch_assoc($s_a);*/
